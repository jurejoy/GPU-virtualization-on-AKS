...
for pod in "${pods[@]}"; do
  ...
  kubectl exec "$pod" -- bash -c "python3 deepfashion_small_batch.py > output_HAMi_small_15pods.txt 2>&1" &
...
counter=1
for pod in "${pods[@]}"; do
  local_filename="output${counter}_HAMi_small_15pods.txt"
  echo "Copying log from $pod to local file: $local_filename"
  kubectl cp "$pod":/home/harry/ResNeSt/output_HAMi_small_15pods.txt "$local_filename"
  ((counter++))
done
