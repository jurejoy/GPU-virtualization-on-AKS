...
for pod in "${pods[@]}"; do
  ...
  kubectl exec "$pod" -- bash -c "python3 deepfashion_small_batch.py > output_MIG_small.txt 2>&1" &
...
counter=1
for pod in "${pods[@]}"; do
  local_filename="output${counter}_MIG_small.txt"
  echo "Copying log from $pod to local file: $local_filename"
  kubectl cp "$pod":/home/harry/ResNeSt/output_MIG_small.txt "$local_filename"
  ((counter++))
done
